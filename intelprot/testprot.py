...
"""
  ...
  Execution in command prompt::
    ...
    >python -m intelprot.testprot -get_setup   # get openspdm_execution.json file in work folder
    >python -m intelprot.testprot -s <openspdm_execution_json> # test in openspdm requester and openspdm responder
    >python -m intelprot.testprot -req cpld -s <openspdm_execution_json>  # test CPLD as spdm requester, openspdm responder
    >python -m intelprot.testprot -res cpld -s <openspdm_execution_json>  # test CPLD as spdm responder, openspdm requester
...
from intelprot import mctp_spdm, spdm
...
class Run_SPDM_Test(object):
  ...
  def setup_test(self, spdmtarget, requester, responder):
    ...
    logfile = '{a}-{b}-req_{c}-res'.format(a=self.tgt, b=self.req, c=self.res)
  ...
  def run_requester(self):
    ...
    if self.req == 'spdm-emu':
      ...
      req_cmdline= self.env[self.tgt]['cmd_spdm_requester_emu'] # cmd_spdm_requester_emu
      if self.res == 'spdm-emu':
        dir = self.env[self.tgt]['spdm_emu_dir']
      if self.res == 'prot':
        dir = self.env[self.tgt]['spdm_emu_req_dir']
  ...
  def run_responder(self):
    ...
    if self.res == 'spdm-emu':
      ...
      res_cmdline= self.env[self.tgt]['cmd_spdm_responder_emu']
      dir = self.env[self.tgt]['spdm_emu_dir']
    ...
    if self.res == 'prot':
      RETRY_GET_VERSION = 3
      recv_get_version  = 0
      while (recv_get_version < RETRY_GET_VERSION+1):
        self.avk = avktool.mctp_avark(0x05)
        # CPLD sends requester mctp in default... waiting for first requester MCTP and
        # continue wait retries of GET_VERSION message,
        # and wait for 'delay_time' to ensure CPLD in responder mode to reponse GET_VERSION
        cpld_s = mctp_spdm.MCTP_CPLD(self.avk.recv())
        if cpld_s.spdm_code == 0x84:
          cpld_s.show()
          recv_get_version += 1
          print("-- received GET_VERSION count: {}".format(recv_get_version))
  ...
  def process_mctp_responder(self):
    ...
    if spdm.dict_Measurements['MeasRecord']['size'] in [0x47, 0x87]:
      ...
      if spdm.dict_Measurements['MeasRecord']['value'].hex() in [LAST_MEASRECORD1, LAST_MEASRECORD2]:
        self.stop_transmit = True
