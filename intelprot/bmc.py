...
import argparse, json, urllib3, requests, pathlib, redfish
...
class Redfish_Update(object):
  ...
  """
  def __init__(self, ipaddress=None, username='debuguser', password='0penBmc1', fbin=None, insecure=False):
  ...
  def watch_task(self, task_id):
    ...
    while True:
      ...
      logger.info('\rUpdating FW{}'.format('.'*(i%4)+' '*(4-(i%4))))
      i += 1
      time.sleep(1)
...
class PFR_Redfish(object):
  """ class for Pfr redfish capsule update

  """
  def __init__(self, bmcip, username, passwd):
    """ init of class """
    self.bmcip
    self.username = username
    self.passwd = passwd

  def update_capsule(self, capsule):
    """ load capsule and update """
...
class BMC_OOB_Mailbox(object):
  ...
  """
  def __init__(self, bmc_ip='10.19.154.198', username='debuguser', passwd='0penBmc1'):
    self.bmc_ip = bmc_ip
...
def main(args):
  ...
  #cmdbuild.ArgumentParser(description="-- build BMC PFR image from manifest json file")
  ...
  # redfish update
  cmdredfish = subparser.add_parser('redfish', description="redfish update")
  cmdredfish.add_argument('-i', '--bmc_ip',   metavar="[BMC IP address]", dest='bmc_ip',   help='BMC IP address')
  cmdredfish.add_argument('-u', '--username', metavar="[username]",       dest='username', help='BMC username')
  cmdredfish.add_argument('-p', '--password', metavar="[password]",       dest='password', help='BMC password')
  cmdredfish.add_argument('-c', '--capsule',  metavar='[capsule]',        dest='upcap',    help='Update capsule')
  cmdredfish.add_argument('-l', '--logfile',  metavar="[logfile]",        dest='logfile',  help='Logfile')
  ...
  if args.action == 'redfish':
    print('do redfish update')
