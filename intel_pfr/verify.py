...
from intel_pfr import keys, pfm, ifwi, bmc
...
class PFM_signed(object):
  ...
  def validate(self):
    ...
    self.logger.info("-- PASS") if result else self.logger.info("-- FAIL")
...
class Verify_PFM(object):
  ...
  :param logfile: logfile, optional, default is None

  """
  def __init__(self, image_bdata, logfile = None):
    self.logger = logging.getLogger(__name__)
    self.pfmobj = pfm.PFM(image_bdata, logfile)
  ...
  def validate(self):
    ...
    rtn = True
    pre_end = 0x0
    print("-- lst_spi_region_sorted:")
    for i in lst_region_sorted:
      print("**** 0x{:08x} --> 0x{:08x}".format(i[0], i[1]))
      rtn &= (i[0] == pre_end)
      pre_end = i[1]
    rtn &= (pre_end == 0x04000000) or (pre_end == 0x08000000)
    return rtn
...
class Capsule_signed(object):
  ...
  def validate(self):
    ...
    self.logger.info('validate block: ', rtn)
...
class Capsule_unsigned(object):
  ...
  def validate(self):
    ...
    self.block = BLOCK(self.blk_data, self.pc_data, self.pfr_version, self.rk_prv, self.csk_prv)
    result = self.block.validate()
    self.logger.info("-- validation result:", result)
    return result
...
class PFR_IFWI(object):
  ...
  def validate(self):
    ...
    act, rcv = int(self.prov._pfrs['pch_active'], 0), int(self.prov._pfrs['pch_recovery'], 0)
    print("act = 0x{:08x}, rcv = 0x{:08x}".format(act, rcv))
    ...
    if self.rk_key is not None:
      ...
      print('calc_rkhash: {}'.format(self.calc_rkhash))
      print('keym_rkhash: {}'.format(self.keym_rkhash))
      rtn &= (self.calc_rkhash == self.keym_rkhash)

    # verify signature
    if self.rk_key is not None and self.csk_prv is not None:
      verify_sign = True
      verify_sign &= BLOCK_1K_Signed(pfm_act, self.rk_key, self.csk_prv, self.logfile).validate()
      verify_sign &= BLOCK_1K_Signed(pfm_rcv, self.rk_key, self.csk_prv, self.logfile).validate()
      rtn &= verify_sign
...
class Verify_AFM(object):
  ...
  :param logfile: logfile, optional
  ...
  def validate_afm_struct_header(self):
    ...
    rtn &= pc_type == 0x6
    obj = BLOCK_1K_Signed(self.bdata[start_offset:], self.rk_key, self.csk_prv, self.logfile)
    rtn &= obj.validate()
  ...
  def validate_afm_device(self, idx):
    ...
    rtn = BLOCK_1K_Signed(self.bdata[(start_offset-0x400):], self.rk_key, self.csk_prv, self.logfile).validate()
  ...
  def validate(self):
    """ verify AFM capsule """
    # verify signature
    rtn  = BLOCK_1K_Signed(self.bdata, self.rk_key, self.csk_prv, self.logfile).validate()
    print("1-RTN:{}".format(rtn))
    """
...
class BLOCK_1K_Signed(object):
  ...
  A genric class to verify all kinds of 1024 Bytes Block signed data verification
  ...
  def verify_pc_hash(self):
    ...
    self.logger.info('hash256 : block0.hash256 {:s}'.format(self.blk_dict['b0_hash256'].hex()))
    self.logger.info('hash256 : pcdata.hash256 {:s}'.format(hash256))
    self.logger.info('hash384 : block0.hash384 {:s}'.format(self.blk_dict['b0_hash384'].hex()))
    self.logger.info('hash384 : pcdata.hash384 {:s}'.format(hash384))
    rtn = True if ((self.blk_dict['b0_hash256'].hex() == hash256) and (self.blk_dict['b0_hash384'].hex() == hash384)) else False
    self.logger.error('FAIL -- verify_pc_hash error') if (not rtn) else self.logger.info('PASS -- verify_pc_hash')
    return rtn
  ...
  def verify_magic(self):
    ...
    self.logger.info('-- FAIL: verify Magic') if (not rtn) else self.logger.info('PASS: verify Magic')
    return rtn

  def verify_pubkey_xy(self):
    """ verify root public key component X and Y """
    if keys.get_eckey_type(self.rk_key) is 'private':
      rkobj = keys.PrivateKey().read_from_pem(self.rk_key)
    if keys.get_eckey_type(self.rk_key) is 'public':
    ...
    self.logger.info('-- FAIL: verify public key X, Y') if (not rtn) else self.logger.info('PASS: verify public key X, Y')
    return rtn
  ...
  def verify_signature(self):
    ...
    if keys.get_eckey_type(self.rk_key) is 'private':
      ...
      self.logger.info('from image: csksigr  = {:s}, \n csksigs = {:s}'.format(self.blk_dict['b1c_sigR'].hex(), self.blk_dict['b1c_sigS'].hex()))
      self.logger.info('FAIL -- verify csk signature') if (not rtn) else self.logger.info('PASS -- verify csk signature')
    ...
    self.logger.info('from image: blk0sigr = {:s}, blk0sigs = {:s}'.format(self.blk_dict['b1b_sigR'].hex(), self.blk_dict['b1b_sigS'].hex()))
    self.logger.info('FAIL -- verify block0 signature') if (not rtn) else self.logger.info('PASS -- verify block0 signature')
    return rtn
...
def main(args):
  ...
  if args.action == 'capsule':
    BLOCK_1K_Signed(args.input_bin, args.rk_key, args.csk_prv, args.logfile).validate()

  if args.action == 'afm':
    obj=Verify_AFM(args.input_bin, args.rk_key, args.csk_prv, args.logfile)
    obj.validate()

  if args.action == 'pfm':
    obj=Verify_PFM(args.input_bin, args.logfile)
    obj.validate()
