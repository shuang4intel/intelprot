...
class PFM(object):
  ...
  def __init__(self, image_bdata, logfile = None):
    ...
    if self.logfile != None:
      logging.basicConfig(level=logging.DEBUG,
        handlers= [
                   logging.FileHandler(self.logfile, mode='w'),
    ...
    for i in range(0, fvm_idx):
      ...
      fvm_addr = fvm_dict['fvm_addr'] - PFR_PFM_OFFSET
  ...
  def show_spi(self, fmt='orgtbl'):
    ...
    for i in range(0, self.fvm_cnt):
      fv_type = self.pfm_dict['fvm']['{:d}'.format(i)]['head']['fv_type']
      if fv_type == 0x0: fv_type = 'bios'
      if fv_type == 0x1: fv_type = 'me/sps'
      if fv_type == 0x2: fv_type = 'microcode 1'
      if fv_type == 0x3: fv_type = 'microcode 2'
      if fv_type == 0xA: fv_type = 'sps oper 1'
  ...
  def show_fvm(self, fmt='orgtbl'):
    ...
    msg = tabulate.tabulate(lst_v, fvm_header, tablefmt=fmt)
    logger.info('\n'+msg)
...
def main(args):
  ...
  #print(args)
  mypfm = PFM(args.input_bin, args.logfile)
  mypfm.show()
