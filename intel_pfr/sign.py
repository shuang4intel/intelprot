...
"""
  ...
  >python -m intel_pfr.sign -h   # get help
  >python -m intel_pfr.sign -i <input_unsigned_image> -t <pc type> -n <csk_id> -r <root private key> -c <csk private key> -o <output_signed_image>
...
def get_csk_entry_hash(permission, csk_id, csk_pub, hashfunc='sha384', outfile='csk_entry_hash.bin'):
  ...
  if hashfunc is 'sha384': curvemagic = 0x08F07B47
  if hashfunc is 'sha256': curvemagic = 0xC7B88C74
...
def main(args):
  ...
  ..example::
   ...
   >>> from intel_pfr import pfr_sign
   >>> pfr_sign.main(['-i', input_image, 't', pc_type, '-v', 'n', csk_id, 'r', rk_prv, 'c', csk_prv])
   or:
   >>> x=pfr_sign.Signing(input_image, pc_type, csk_id, rk_prv, csk_prv)
   >>> x.sign()

   For external signing::

    >python -m intel_pfr.sign -extsign -h
    >python -m intel_pfr.sign extsign -i PFM_signed.bin -t 3 -n 2 -rk key_root_pub.pem -csk key_csk_prv.pem \
